...
def allocate_history(request, credit_pk=None):
	if credit_pk:
		credit = Allocate.objects.get(pk=credit_pk)
...
def allocate_detail_pdf(request, pk=None):
	try:
		credit = Allocate.objects.get(pk=pk)
		all_sales = CreditHistoryEntry.objects.filter(credit=credit)
		img = default_logo()
		data = {
			'today': date.today(),
...
def allocate_list(request):
	try:
		...
		all_sales = Allocate.objects.filter(created__contains=last_date_of_sales)
...
@staff_member_required
@permission_decorator('reports.view_sale_reports')
def allocate_reports(request):
	try:
		today = datetime.date.today()
		items = AllocatedItem.objects.all().order_by('-id')
		ts = Allocate.objects.filter(created__icontains=today)
		tsum = ts.aggregate(Sum('total_net'))
		total_sales = Allocate.objects.aggregate(Sum('total_net'))
		total_tax = Allocate.objects.aggregate(Sum('total_tax'))
		page = request.GET.get('page', 1)
...
@staff_member_required
def allocate_paginate(request):
	page = int(request.GET.get('page'))
	list_sz = request.GET.get('size')
	p2_sz = request.GET.get('psize')
	select_sz = request.GET.get('select_size')
	date = request.GET.get('gid')
	sales = Allocate.objects.all().order_by('-id')
	...
	if date:
		try:
			...
			total_sales_amount = Allocate.objects.filter(created__contains=date).aggregate(Sum('total_net'))
			sales = []
			...
			if p2_sz and date:
				...
				return TemplateResponse(request,'dashboard/reports/allocate/paginate.html',{'sales':sales, 'gid':date})
			...
			return TemplateResponse(request, 'dashboard/reports/allocate/p2.html',{'date': date})
		...
		try:
			...
			if p2_sz:
				paginator = Paginator(sales, int(p2_sz))
				sales = paginator.page(page)
				return TemplateResponse(request,'dashboard/reports/allocate/paginate.html',{'sales':sales})
